...
const App: React.FC = () => {
  ...
  // Robust path normalization for the switch-based router
  const normalizedPath = useMemo(() => {
    return pathname.replace(/^\/+/, '').split('?')[0] || 'landing';
  }, [pathname]);

  const currentPage = normalizedPath;
  const selectedTemplateId = searchParams.get('template') || 'yuki-blue';
  ...
  const navigate = useCallback((page: string) => {
    // Ensure path starts with / for the router push
  ...
  const handleMethodSelect = useCallback((method: 'upload' | 'scratch') => {
    if (method === 'upload') {
      // FIX: Navigating to 'upload-method' directly instead of 'analyzer' (which triggers a redirect loop)
      navigate(`upload-method?template=${selectedTemplateId}`);
    } else {
      setPendingFile(null);
  ...
  const handleFileUpload = useCallback((file: File) => {
    setPendingFile(file);
    navigate(`direct-port?template=${selectedTemplateId}`);
  }, [navigate, selectedTemplateId]);
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'builder': 
        return <Builder user={user || undefined} initialTemplateId={selectedTemplateId} prefilledData={prefilledData || undefined} />;
      case 'dashboard': 
      ...
      case 'faq': 
        return <FAQ />;
      case 'resume-option': 
        return <ResumeOption onSelect={handleMethodSelect} templateId={selectedTemplateId} />;
      case 'upload-method': 
        return (
          <UploadMethod 
            onFileSelect={handleFileUpload}
            onBack={() => navigate(`resume-option?template=${selectedTemplateId}`)}
          />
        );
      case 'direct-port': 
        return (
                    ...
                    <div className="aspect-[3/4] overflow-hidden relative rounded-[2.5rem] border border-slate-200 shadow-[0_10px_40px_rgb(0,0,0,0.04)] transition-all duration-500 group-hover:shadow-[0_30px_80px_rgb(0,0,0,0.12)] group-hover:border-blue-300 bg-white group-hover:-translate-y-3">
                      ...
                      <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center space-y-4 px-10">
                        <button 
                          ...
                          className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 hover:opacity-90 flex items-center justify-center space-x-2"
                        >
